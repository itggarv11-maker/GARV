    ...
    const renderForm = () => (
        <Card variant="light" className="max-w-2xl mx-auto">
            <div className="text-center mb-8">
                ...
                <h1 className="text-3xl font-bold text-slate-800 mt-4">Interactive Mind Map Generator</h1>
                <p className="mt-2 text-slate-600">Visualize any topic to understand it better. Click nodes to expand and collapse.</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    <label htmlFor="topic" className="block text-sm font-medium text-slate-700">What topic do you want to map?</label>
                    <input
                        type="text"
                        id="topic"
                        value={topic}
                 ...
                 <div>
                    <label htmlFor="classLevel" className="block text-sm font-medium text-slate-700">Select your class level</label>
                    <select
                        id="classLevel"
                        value={classLevel}
        ...
        if (isLoading && !mindMapData) {
            return (
                 <div className="flex flex-col items-center justify-center gap-4 py-16">
                    ...
                    <p className="text-slate-600">Generating your mind map...</p>
                </div>
            );
        }
        if (mindMapData) {
            // Once data is available, hide the main loader and let the component handle its internal state.
